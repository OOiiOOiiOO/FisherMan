...
## ANSI colors (FG & BG)
RED="$(printf '\033[31m')"  GREEN="$(printf '\033[1;92m')"  ORANGE="$(printf '\033[3;49;92m')"  BLUE="$(printf '\033[34m')"
...
banner() {
	cat <<- EOF
		...
		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tool Created by ooiioooiioo${WHITE}
	EOF
}
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} Ba Arezoy Lavat Hay Bishtar :).${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN}Author   ${RED}:  ${GREEN}LAVAT GAR AZAM ${RED}[ ${GREEN}ooiioooiioo ${RED}]
		${GREEN}Github   ${RED}:  ${RED} https://github.com/OOiiOOiiOO 


		${GREEN}License  ${RED}:  ${CYAN}GNU General Public License v3.0
		...
		${RED}[${WHITE}00${RED}]${GREEN} Main Menu     ${RED}[${WHITE}99${RED}]${GREEN} Exit

	EOF

	read -p "${RED}[${WHITE}-${RED}]${WHITE} Select an option : ${RED}"
...
main_menu() {
	...
	cat <<- EOF
		...
		${RED}[${WHITE}34${RED}]${ORANGE} Discord	${RED}[${WHITE}35${RED}]${ORANGE} Telegram	${RED}[${WHITE}36${RED}]${ORANGE} Pornhub
	...
	case $REPLY in 
		...
		35)
			website="telegram"
			mask='http://get-telegram-bot-free'
			tunnel_menu;;

		36)
			website="pornhub"
			mask='http://get-pornhub-premium-account-free'
			tunnel_menu;;
